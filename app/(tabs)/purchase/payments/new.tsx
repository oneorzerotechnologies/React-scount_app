...
import { MOCK_EXPENSE_CATEGORIES } from '@/constants/mock-categories';
...
export default function NewExpenseScreen() {
  ...
  const [categoryId,  setCategoryId]  = useState(MOCK_EXPENSE_CATEGORIES[0].id);
  const [accountId,   setAccountId]   = useState(MOCK_CASH_ACCOUNTS[1].id);
  ...
  const [pickerOpen,   setPickerOpen] = useState<null | 'category' | 'account' | 'method'>(null);

  const category = MOCK_EXPENSE_CATEGORIES.find((c) => c.id === categoryId)!;
  const account  = MOCK_CASH_ACCOUNTS.find((a) => a.id === accountId)!;
  ...
  const onSave = () => {
    ...
    console.log('Save expense', {
      date, category, paid_from: account, payment_option: method,
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: palette.background }]} edges={['top']}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <Section label="Paid from" palette={palette}>
            <PickerRow
              palette={palette}
              label={account.name}
              sub={account.type.toUpperCase()}
            ...
            >
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.taxHint, { color: palette.textMuted }]}>
                  Toggle off for tax-exempt expenses.
                </Text>
              </View>
      ...
      <OptionPicker
        ...
        options={MOCK_EXPENSE_CATEGORIES.map((c) => ({ value: c.id, label: c.name, sublabel: `Code ${c.code}` }))}
        selected={categoryId}
      ...
      <OptionPicker
        ...
        title="Paid from"
        options={MOCK_CASH_ACCOUNTS.map((a) => ({ value: a.id, label: a.name, sublabel: a.type.toUpperCase() }))}
        selected={accountId}
        onSelect={(v) => { setAccountId(v); setPickerOpen(null); }}
        onClose={() => setPickerOpen(null)}
      />
      <OptionPicker
